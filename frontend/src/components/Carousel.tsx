...
const Carousel = forwardRef(
  (
    ...
    const { user } = useUser();
    const [showManualAddDisclaimer, setShowManualAddDisclaimer] =
      useState(false);
    ...
    const [capstoneCourseId, setCapstoneCourseId] = useState<string | null>(
      null
    );
    ...
    const [hasTransferredSessionData, setHasTransferredSessionData] =
      useState(false);
    const [manualCourseCodes, setManualCourseCodes] = useState<Set<string>>(
      new Set()
    );
    ...
    useEffect(() => {
      const fetchCapstones = async () => {
        if (!user?.id) {
          ...
          const requirements =
            concentrationUtils.getRequirements(concentration);
      ...
      const fetchData = async () => {
        ...
        try {
          const response = await fetch(
            `${
              import.meta.env.VITE_BACKEND_URL
            }/get-user-courses-detailed?uid=${user.id}`
          );
            ...
            for (const [semester, courseList] of sortedSemesters) {
              ...
              for (const course of courseList) {
                ...
                newCourses.push({
                  ...
                  semesterId: semester,
                  isEditing: false,
                  prereqsMet: isManualCourse
                    ? undefined
                    : course.prereqsMet ?? undefined,
      ...
      const updatedCourses = await new Promise<CourseItem[]>((resolve) => {
        setCourses((prev) => {
          const updated = prev.map((c) =>
            c.id === id
              ? { ...c, courseCode, title, isEditing: false, isManual: true }
              : c
          );
          resolve(updated);
          return updated;
                ...
                {boxSelections[boxId] &&
                  getCoursesForSemester(boxSelections[boxId]).map((course) => (
                    <CourseDrag
                      ...
                      isCapstone={course.isCapstone ?? false}
                      showCapstoneCheckbox={capstoneCodes.has(
                        course.courseCode
                      )}
        ...
        {showManualAddDisclaimer && (
          ...
          >
            <div className="disclaimer-box">
              ...
              <p>
                You're manually adding a course. Enter course code and course
                name for Non-CS courses, and hit Enter to save this
                manually-added course. Please note that these courses will not
                be tracked on your concentration progression meter.
              </p>
            </div>
          </div>
        )}
