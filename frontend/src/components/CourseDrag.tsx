...
export default function CourseDrag({
  ...
  const handlePrereqClick = async (e: React.MouseEvent) => {
    ...
    console.log("Prerequisites button clicked for:", courseCode, "userId:", userId, "isManual:", isManual);
    ...
    try {
      ...
      if (!userId) {
        ...
        if (userId && (data as any).response_type && (data as any).response_type !== "success") {
  ...
  return (
    <div
      className={`
        ...
        ${!isEmpty && prereqsMet !== undefined ? (prereqsMet ? "pr-met" : "pr-not-met") : ""}
        ${isCapstone ? "capstone" : ""}
